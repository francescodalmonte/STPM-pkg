...
if __name__ == "__main__":
    ...
    save_dir = "D:/data/ds_CABE_yyr_Vtile/results_224/ds_mode0_wcorr/inference"   ######## TODO change this
    ...
    contrast_correction_sigma = float(params["CONTRAST_CORRECTION_SIGMA"])

    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"Running on device: {device}")

    dirnames = [d for d in os.listdir(input_path) if d.endswith(".obj")]
    for i, dirname in enumerate(dirnames):
        input_name = dirname[:-4]
        save_path = os.path.join(save_dir, input_name)
        print(f"Processing {input_name}... {i}/{len(dirnames)}")
